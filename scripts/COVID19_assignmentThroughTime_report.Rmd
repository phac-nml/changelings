...
---
 ...
 The script in this markdown is used to generate the Sankey visualization of changes in Pangolin lineage assignment at any point in specificied timepoints, and report top changes in counts and proportions of lineages. Updated to be compatible with Pangolin v4.0.
...
linlist<-lapply(pdates,function(pdate){
  ...
  if(file.exists(file.path(datDir, "lineage_report.csv"))){
    ...
    if("pangoLEARN_version"%in%colnames(oplearn)){## before pangolin v4.0
      vers<-paste(pdate, "pangolin:",oplearn$pangolin_version[1],
                  "pangoLEARN:",oplearn$pangoLEARN_version[1],
                  "pango:",oplearn$pango_version[1])
      ...
      vers<-paste(pdate, "pangolin:",oplearn$pangolin_version[1],
                  "version:",oplearn$version[1], 
                  "scorpio:",oplearn$scorpio_version[1],
                  "constell.:",oplearn$constellation_version[1])
...
if(nrow(diffMat)!=0){ ## if there are changes
  ...
  ### setting height, with a maximum, adding min of 50 on 2022-03-11
  hei=max(50,ifelse(nrow(datVis)/1.5*10 > maxSankeyPx, maxSankeyPx, nrow(datVis)/1.5*10))
...
if(nrow(diffMat)!=0){ ## if there are changes from any pairs
  ...
  hist(linpercchange, breaks=30, main="Overall % Changes between consecutive runs", xlab="Percent change",xlim=c(0,100))
...
if(nrow(diffMat)!=0){ ## if there are changes in any pairs of time points
  ...
  scale_fill_manual(values = col2use, breaks = levels(changeState[[y]])) 
    ...
    }else if (nrow(cpt)==1){ ## no ordering
