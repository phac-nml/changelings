...
linlist<-lapply(pdates,function(pdate){
  ...
  if(file.exists(file.path(datDir, "lineage_report.csv"))){
    ...
    vers<-paste(pdate, "pangolin:",oplearn$pangolin_version[1],"pangoLEARN:",oplearn$pangoLEARN_version[1], "pango:",oplearn$pango_version[1])
...
if(nrow(diffMat)!=0){ ## if there are changes in any pairs of time points
  ...
  changeState<-lapply(changePairs,function(cp0){
    ...
    if(length(cp0)==0){
      ...
      ### if the pairs are siblings, or contained None, or "others"
    ...
    factor(changeState0, levels=c("child","parent","sibling","other","None"))
  })
